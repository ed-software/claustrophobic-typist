...
if (is_string(keyboard_lastchar) && is_string(global.target_text)) {
	var lastchar = keyboard_lastchar;
	
	if (global.target_text == undefined || global.target_text == "undefined") {
		global.target_text = "";	
	}
	...
	if (global.target_text == "") {
		...
		global.shrinkBy = sqrt(global.index) / (1000*25);
		global.growByFullWord = string_length(global.target_word) / 50;

		global.index += floor(random_range(0, 500 + power(global.difficulty, 2)*3000));
		randomize();
